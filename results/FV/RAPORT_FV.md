...
## §3. O4 — złożenie osłona∘sieć (stretch (ii), niebramkujące)
...
- **L3 — `results/FV/NOTA_UNROLL.md` (desk-note, status „częściowy").** Weryfikacja unroll-T wnętrza CfC:
  co by wymagała (rozwinięcie T kopii rekurencji, kodowanie tanh/σ przestępnych, związanie 1903 wag
  numpy, własność warta dowodu), klasy narzędzi z nazwy (dReal δ-zupełne; CROWN/auto_LiRPA propagacja
  granic; Verisig/NNV/POLAR/ReachNN osiągalność; Marabou/Reluplex MILP — statusy z dokumentacji,
  do weryfikacji), dlaczego poza budżetem (F6/F7/SR-FV-3: nowa zależność, GPU/toolchain). Własność
  bezpieczeństwa (|y|≤V_MAX) jest JUŻ zamknięta architektonicznie przez L1 bez unrollu.
...
## §6. PROPOZYCJA kanonu roszczeń FV (NIE obowiązujący — finalizacja w ANEKS_FV-2)

> **WOLNO** mówić: „D5 (`safe_descend_step`) ma dowiedzione w z3, na lustrze 1:1 zwalidowanym różnicówką
> (0 rozbieżności) i M-krokiem (27/27), własności (a)-(d) przy jawnym założeniu monotonicznego zegara
> — cert P6_d5"; „histereza `_pos_monitor` ma wyczerpaną enumerację grafu (103/206) z dowiedzionymi
> progami debounce=2 / hyst=100 / brakiem trzepotania (cykl 102) — cert P7_posmon"; „predykat
> `_geofence_violation` zawiera się w barierze certu P2 na domenie clampu |v|≤v_max — cert P8_geo_cont,
> niebramkujący"; „wyjście sieci jest architektonicznie ograniczone |y|≤V_MAX (tanh·V_MAX) i domknięte
> klampem `clip_v` — lematy L1/L2"; „każde roszczenie z DOMENĄ i ZAŁOŻENIAMI wypisanymi w cercie".
>
> **NIE WOLNO** mówić: „system zweryfikowany formalnie" / „formally verified system" (zdanie nie
> istnieje — PRE_FV §5); „osłona dowiedziona bezpieczna w locie" (dowody dotyczą KODU i MODELU, nie
> rzeczywistości lotu — brak twierdzeń o dynamice/fizyce/środowisku); „sieć CfC zweryfikowana" (wnętrze
> niedowiedzione — NOTA_UNROLL „częściowy"; dowiedzione tylko ZŁOŻENIE); „predykat pokrywa V_env=6.0"
> (poza zakresem, erratum #2); pomijać założenie „lustro ≡ produkcja" przy cytowaniu któregokolwiek
> certu FV.
...
- **ARCH-1 ANEKS_FV-1a ODROCZONY:** plik `ANEKS_FV-1a.md` nieobecny w Downloads podczas S3.
  C0' (dbc4442) skomitował `ANEKS_FV-1b.md` (sha src↔repo 7a314ee0…); `ANEKS_FV-1a.md` odroczony —
  **prośba do Olgi o dostarczenie do repo** (ARCH-1). Treść operacyjna -1a niesiona przez -1b §5
  (§1–§3 i §6 -1a w mocy). Wcześniej: ANEKS_FV-1.md domknięty do repo commitem 57d8f45 (poprzednia sesja).
