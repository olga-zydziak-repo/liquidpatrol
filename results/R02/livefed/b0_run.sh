#!/usr/bin/env bash
# b0_run.sh <RUN> — B0 sonda live-fed (lot+headless+detektor). Habitat H.1: HEADLESS zweryfikowany, RTF,
...
RUN="${1:-run}"
OUTDIR="$ROOT/results/R02/livefed/B0/$RUN"; mkdir -p "$OUTDIR"
...
[ -z "$IMG" ] && { echo "[b0] BRAK kamery"; teardown; exit 5; }
echo "[b0] kamera: $IMG"
...
echo "[b0] detektor start; 90 s settle EKF + załadowanie wag"; sleep 90
...
OUTDIR="$OUTDIR" INTR_RANGE="$INTR_RANGE" IMG_TOPIC="$IMG" HEADLESS=1 RENDER_BACKEND="$RENDER_BACKEND" \
  PYTHONPATH="$B0SP:$ROOT:${PYTHONPATH:-}" python3 results/R02/livefed/b0_flight.py > "$OUTDIR/b0.log" 2>&1
RC=$?
grep -ci 'time jump\|Resetting time sync' "$OUTDIR/stack.log" > "$OUTDIR/timejump_post.txt" 2>/dev/null || true
teardown
echo "[b0] $RUN DONE rc=$RC → $OUTDIR"; exit $RC
