...
- **[A5] Reguła zejścia po drabinie:** wyczerpanie budżetu §4 **LUB ≥3 pady o tej samej sygnaturze** (`exit 144` / `dxg ioctl -22`) przy **udokumentowanej konfiguracji** → zejście o jeden szczebel fallbacku (§2) **z zapisem sygnatury** w `RAPORT_R0`. Pady o różnych sygnaturach = diagnozować osobno, nie kumulować do progu 3.
  - **Doprecyzowanie (2026-08-05): `exit 144` sam w sobie NIE jest padem GPU.** Liczy się do progu A5 **tylko** gdy towarzyszy mu wpis **`dxg`/`ioctl -22`** (lub równoważny błąd sterownika GPU) w `dmesg` z tego samego okna czasowego. `exit 144` bez takiego wpisu w dmesg (np. propagacja sygnału z grupy procesów przy ubijaniu PX4 startowanego z tła — zaobserwowane w sesji 1) = artefakt zarządzania procesami, **nie** kumulować. Weryfikacja padu = `dmesg | grep -iE "dxg|ioctl"` przy każdym `exit 144`.
  - **Doprecyzowanie #2 (2026-08-05, empiryczne): `dxgkio_query_adapter_info: Ioctl failed: -22` sam w sobie NIE jest padem GPU.** Zmierzono korelację (sesja 2, §3.2): każde wywołanie `nvidia-smi` generuje świeże wpisy `query_adapter_info -22` w dmesg, podczas gdy sim (gz GUI + px4 + render D3D12) pozostaje w pełni zdrowy. To szum zapytań o adapter od narzędzi monitorujących, nie awaria. **Pad GPU do progu A5 = `dxg -22` KOINCYDENTNY ze śmiercią procesu** (`WSL … CaptureCrash`, SIGABRT/exit-144, zatrzymanie renderu). Bare `query_adapter_info -22` bez śmierci procesu = **nie kumulować**. Konsekwencja: podczas soaku (A2) `nvidia-smi` odpytywać rzadko; kryterium pada = śmierć procesu / `CaptureCrash`, nie liczba wpisów `-22`.
